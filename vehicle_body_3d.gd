...
var JumpPower = 10
@onready var area:Area3D = $Node/Area3D
...
func is_on_floor():
	return len(area.get_overlapping_bodies()) > 0

func jump():
	if is_on_floor() and !jumpCooldown and Global.jumpingEnabled:
		jumpCooldown = true
		linear_velocity += Vector3(0,JumpPower,0)
		if get_tree():
			get_tree().create_timer(Global.jumpCooldown).timeout.connect(func():
				jumpCooldown = false
			)
...
func _physics_process(delta):
	...
	var dead = $detect_dead.is_colliding() and not Global.godMode
